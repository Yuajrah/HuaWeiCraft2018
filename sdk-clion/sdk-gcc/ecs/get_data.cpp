...
std::map<int, std::vector<double>> get_esc_data(
        ...
        int hour_split // 以hour_split小时为统计单元, hour_split最好为24*数据天数的因子, 否则最后不足的数据将直接取往前一次的数据代替
){
    std::map<int, std::vector<double>> res;

    // 初始化，给每种vm分配数据存储空间（连续天）
    int periods_len = std::ceil(get_days(start_date, end_date) * 24 / static_cast<double>(hour_split));  // 数据长度
    ...
    for (int i=0;i<data_num;i++) {
        ...
        if (iter != res.end()) {
            if (hour_index >= 0 && get_hours(date, end_date) > 0 ) { // 如果 start_date <= date && date < end_date，则是需要统计period内的数据
                iter->second[hour_index / hour_split]++;
            }
            if (get_days(start_date, end_date) * 24 / static_cast<double>(hour_split) < periods_len) {
